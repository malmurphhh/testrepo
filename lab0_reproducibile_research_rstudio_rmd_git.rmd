...
2 columns, 13 rows. dataset is a character variable, n is a numeric (integer) variable

**Q2.** Uncomment and complete the ggplot code to plot `y` vs `x` for the `dino` subset, and compute the Pearson correlation.

The ggplot makes a dinosaur and Pearson r = -0.06447185	

```{r}
dino_data <- datasaurus_dozen |>
  filter(dataset == "dino")

ggplot(dino_data, aes(x = x, y = y)) + geom_point()
...
Plot makes a star and Pearson r = -0.0629611
```{r}
star_data <- datasaurus_dozen |>
  filter(dataset == "star")

ggplot(star_data, aes(x = x, y = y)) + geom_point()
...
ggplot(circle_data, aes(x = x, y = y)) + geom_point()
...
Despite the visual differences, the reason why these summary statistics are the same is that mean and standard deviation are measures of central tendency and variance around that measure. So for each dataset, once all datapoints are added and divided by the total n, their average values are very similar despite individual datapoints being different;  similarly, the spread of the datapoints around the mean is similar. 
...
Added column with mutate

(c) Compute mean `Ozone`, mean `Temp`, and the count of complete days per month.

Means and counts calculated with summarise and mean = and sum = , respectivdely

(d) Plot `Ozone` against `Temp`, coloured by month, with a smoothed trend line (`geom_smooth(method = "lm")`).

plotted with ggplot and geom_point and geom_smooth

```{r}
aq <- as_tibble(airquality)
aq

#(a) - dropping rows with missing Ozone values
aq <- aq |>
  filter(!is.na(Ozone))
aq

#(b) - adding column month_name
aq <- aq |>
  mutate(
    month_name = month.abb[Month])
aq

#(c) - mean Ozone, mean Temp, count # days/month
aq |>
  summarise(
...
aq|>   group_by(month_name) |> summarise(complete_days = sum(complete.cases(Ozone, Temp)))

#d - plot Ozone by Temp, smoothed
ggplot(aq, aes(x = Ozone, y = Temp, colour = month_name)) + geom_point() + geom_smooth(method = "lm", se = FALSE)
